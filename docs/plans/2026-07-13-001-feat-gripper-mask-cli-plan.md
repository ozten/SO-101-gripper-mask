...
- KTD-4. **Component selection = Lab chroma-core gate; keep every passing component.** First-round QA on the full dataset falsified both the ranked-selection design and HSV as the discriminating space: the warm wall is *more* HSV-saturated than the gripper in some frames, and the keep-1-2 rule dropped finger fragments (motion blur splits a finger into several blobs) and fingers merged into wall components. What separates reliably is Lab chroma ("vibrancy"): measured gripper components sit at chroma 59–83 (49–88% of pixels ≥ 60) while the wall/wood measure 46–50 (0–23%) and corks 23–43 (0%). Selection: restrict to `--roi` when given; drop components below `--min-area` (default 0.5% of frame pixels); keep every component whose vivid fraction (share of pixels with chroma ≥ `--core-chroma`, default 60) is ≥ `--core-frac` (default 0.35); for components below the gate that still contain gripper-sized vivid sub-regions (a finger visually abutting the wall merges into its component), cut out and keep the vivid sub-regions. Keeping all passers is deliberate: over-masking a vivid orange scene object is acceptable, losing gripper pixels into training data is not.
- KTD-5. **Solid masks: filled contours + grow + hole-fill.** Kept components render as filled contour interiors; the final mask is then grown outward by `--grow` (default 5 px) and every enclosed hole is flood-filled. Downstream gaussian-splat training corrupts on pinholes inside the gripper region, so the mask is guaranteed solid — growing beyond the gripper is acceptable, missing any of it is not (user-confirmed priority). The QA overlay uses a translucent tint rather than solid fill so growth and bleed onto grasped objects stay visible.
...
- Default tuning values were frozen against the reference dataset after user QA: hue ±8 OpenCV units, S ≥ 100, V ≥ 60, 5×5 close ×2, 3 px dilate, chroma core ≥ 60 at fraction ≥ 0.35, grow 5 px. The earlier warm-wood limitation was resolved by the Lab-chroma redesign (KTD-4); the remaining accepted over-mask is vivid orange scene objects near the gripper's chroma.
